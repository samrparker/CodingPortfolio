...
using static MathNetUtils;
using Random = UnityEngine.Random;

public class Glass : MonoBehaviour
{
    private const float MicroShardSurface = 0.07f;
    private const float MicroShardTimer = 1f;
    private const float SmallShardSurface = 0.15f;
    private const float SmallShardTimer = 2f;
    private const float Tolerance = 0.001f;
    public Vector3 originalPosition;
    public int glassShatterLevel = 0;
    ...
    {
        ThrowableObject thrownObject = collision.gameObject.GetComponent<ThrowableObject>();

        if (thrownObject != null)
        {
            if (isShard || thrownObject.DestroyGlass)
            {
                Destroy(gameObject);
            }
            Rigidbody throwBody = collision.gameObject.GetComponent<Rigidbody>();
            ContactPoint firstContact = collision.GetContact(0);
            Break(firstContact.point, firstContact.normal * 0.0f);
            thrownObject.ReplaceVelocity();
        }
    }*/
    ...
    void OnTriggerEnter(Collider collider)
    {
        ...
        ThrowableObject thrownObject = collideObject.GetComponent<ThrowableObject>();
        if (thrownObject == null)
        {
            thrownObject = collideObject.transform.parent.gameObject.GetComponent<ThrowableObject>();
        }
        Collider thisBox = GetComponent<MeshCollider>();
        if (thisBox == null)
        {
            Debug.Log("AA");
        }
        else
        {
            if (thrownObject != null)
            {
                Debug.Log(thisBox.ClosestPoint(collider.gameObject.transform.position));
            }
        }

        if (thrownObject != null)
        {
            //thrownObject.DisableGlassCollision();
            //if (this.glassShatterLevel >=2 || thrownObject.DestroyGlass)
            //{
             //   //Debug.Log("DESTROYING");
             //   return;
            //}
            Rigidbody throwBody = collideObject.GetComponent<Rigidbody>();
            Debug.Log("Breaking!");
            Break(thisBox.ClosestPoint(collider.gameObject.transform.position), new Vector3(0, 0, 0));
            //Break(thisBox.ClosestPointOnBounds(collideObject.transform.position), new Vector3(0, 0, 0)); //WARNING: THIS WILL FREEZE UNITY
            thrownObject.ReplaceVelocity();
        }
    }
    ...
    Polygon2D BuildPolygon(float side)
    {
        ...
        //Debug.Log("POINT AMOUNT 1: " + targetPoints.Count);
        targetPoints.RemoveAll(p => Mathf.Abs(p.Z - side) > Tolerance); // Discard backface
        //Debug.Log("POINT AMOUNT 2: " + targetPoints.Count);
        targetPoints = targetPoints.Distinct(new Point2DComparer(Tolerance)).ToList(); // Discard side submesh vertex duplicates
        //Debug.Log("POINT AMOUNT 3: " + targetPoints.Count);
        foreach (var point in targetPoints)
            point.TransformBy(scalingMatrix);
            
        // Build convex polygon
        targetPoints.Sort((a, b) => CompareVectorAngle(new Point2D(0, 0), a, b));
        //Debug.Log("JESSE" + targetPoints.Count);
        var targetToPolygon = targetPoints.Select(p => p.Point2D);
        //Debug.Log("JESSE" + targetToPolygon.Count());
        Polygon2D targetPolygon = new Polygon2D(targetToPolygon);
    ...
    {
        float shardSurface = mesh.bounds.size.x * mesh.bounds.size.y * 0.05f;
        ...
        meshCollider.excludeLayers = LayerMask.GetMask("TransparentFX");
        meshCollider.isTrigger = true;
        meshCollider.providesContacts = true;

        Glass glass = null;
        if (shardSurface > SmallShardSurface * 10)
        {
            glass = go.AddComponent<Glass>();
            glass.glassShatterLevel = this.glassShatterLevel + 1;
            glass.Patterns = Patterns;
            go.transform.parent = transform.parent; 
            var shardRigidbody = go.AddComponent<Rigidbody>();
            shardRigidbody.mass = 0f;
            shardRigidbody.useGravity = false;
            shardRigidbody.velocity = new Vector3(0.0f, 0.0f, 0.0f);
            shardRigidbody.angularVelocity = new Vector3(0.0f, 0.0f, 0.0f);
            //shardRigidbody.freezeRotation = true;
            shardRigidbody.collisionDetectionMode = CollisionDetectionMode.ContinuousDynamic;
            shardRigidbody.excludeLayers = LayerMask.GetMask("TransparentFX");
        }
        else
        {
            glass = go.AddComponent<Glass>(); 
            Destroy(go);
            //KEEP THIS CODE, I would like to re-implement smaller shards later!
            /*glass.isShard = true;
            glass.Patterns = Patterns;
            go.transform.parent = transform.parent;
            var shardRigidbody = go.AddComponent<Rigidbody>();
            shardRigidbody.mass = 0.0001f;
            shardRigidbody.excludeLayers = LayerMask.GetMask("ThrowableObj");
            shardRigidbody.AddForce(originVector);
            Destroy(go, shardSurface > MicroShardSurface ? SmallShardTimer : 1); // destroy small shards after x seconds*/
        }

        //Debug.Log("New shard generated from glass/shard at position:(" + this.transform.position + ") with position:(" + go.transform.position +")");
